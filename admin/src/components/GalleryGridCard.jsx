...
export const GalleryGridCard = ({ item, active = true }) => {
  return (
    <>
      <div className={!active ? "sr-only" : null}>
        <div className={`aspect-5/4 bg-uranian-blue-1000 p-2 relative border-3 ${item?.published ? "border-sage" : "border-cordovan"}`}>
        ...
        <div className="flex justify-between">
          <div className="p-1">
            <h3>
              {item.title || "(untitled)"}
              {/* <pre className="text-xs">Slug: {item.slug}</pre> */}
            </h3>
          </div>
