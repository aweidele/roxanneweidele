...
export const ImageUploadCard = ({ file }) => {
  ...
  return (
    <div className="w-full mt-2 mb-9 p-4 border border-rose-quartz flex gap-2 transition-all duration-300" style={submitting}>
      ...
      <div className="grow-1">{!file.loading ? <ImageForm artworkId={file.insertID} submitAction={handleSubmit} /> : <div className="w-full h-full flex justify-center items-center">Uploading</div>}</div>
    </div>
  );
};
